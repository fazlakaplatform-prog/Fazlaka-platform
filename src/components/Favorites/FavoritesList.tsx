...
// تعريف واجهات للبيانات (تم تحديثها لتتوافق مع Prisma)
interface Episode {
  id: string;
  title: string;
  titleEn: string | null;
...
export default function FavoritesList() {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const episodeIds = favoritesData
              ...
              .map((fav: Favorite) => fav.episode!.id);
            
            const articleIds = favoritesData
              .filter((fav: Favorite) => fav.article)
              .map((fav: Favorite) => fav.article!.id);
  ...
  const getLocalizedText = (arText?: string | null, enText?: string | null) => {
    return language === 'ar' ? (arText || '') : (enText || '');
  };

  const getThumbnailUrl = (arUrl?: string | null, enUrl?: string | null) => {
      ...
      {favorites.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {favorites.map((favorite) => {
            ...
            return (
              <div key={favorite.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
