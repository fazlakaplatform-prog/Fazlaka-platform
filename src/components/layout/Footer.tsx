...
// واجهة للروابط الاجتماعية - تم تغيير _id إلى id
export interface SocialLink {
  id: string // تم التصحيح من _id
  platform: string
  url: string
  createdAt?: Date
  updatedAt?: Date
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          >
            <div className="flex flex-col items-center text-center">
              ...
              {/* الروابط الاجتماعية - تم تصحيح الـ key هنا */}
              {!loading && validSocialLinks.length > 0 && (
                <motion.div 
                ...
                >
                  {validSocialLinks.map((social, index) => {
                    ...
                    return (
                      <motion.a
                        key={social.id} // <--- تم التصحيح من social._id إلى social.id
                        href={social.url}
                        target="_blank"
          ...
          >
            ...
            >
              ...
              >
                <div className="flex-1 h-12 rounded-xl bg-blue-500/20 flex items-center justify-center">
            ...
            >
              {policyLinks.map((link, index) => (
                ...
                >
                  <Link
                    href={link.href}
                    className={`text-gray-400 hover:text-white transition-all duration-300 text-sm font-medium px-6 py-2 rounded-lg hover:bg-gradient-to-r ${link.gradient} relative group shadow-md`}
                  >
                    {link.text}
                    <div className="absolute inset-0 rounded-lg border border-transparent group-hover:border-white/30 transition-all duration-300"></div>
                  </Link>
                </motion.div>
              ))}
            </motion.div>
