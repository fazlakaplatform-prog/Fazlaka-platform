...
export default function Navbar() {
  ...
  // Admin check
  const isAdmin = session?.user?.role === 'EDITOR' || session?.user?.role === 'OWNER' || session?.user?.role === 'ADMIN';
      ...
      <nav className="hidden md:flex fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-6xl bg-white/60 dark:bg-gray-900/60 backdrop-blur-lg shadow-lg rounded-2xl border border-white/20 dark:border-gray-700/30 py-1.5 px-4 transition-all duration-300">
        <div className="flex justify-between items-center w-full">
          ...
          <div className="flex items-center space-x-1">
            ...
            <motion.button
              onClick={() => router.push("/chat_friends")}
              className={`p-1.5 rounded-lg transition-all duration-300 ${
                isActive("/chat_friends") 
                  ? "bg-indigo-100/50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400" 
                  : "hover:bg-gray-100/30 dark:hover:bg-gray-800/30"
              }`}
              whileHover={{ scale: 1.1 }}
              ...
              aria-label="Chat"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${isActive("/chat_friends") ? "text-indigo-600 dark:text-indigo-400" : "text-gray-700 dark:text-gray-300"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  <div className="space-y-1">
                    ...
                    {[
                      ...
                      { href: "/chat_friends", icon: "chat", label: t.chat, color: "from-green-400/60 to-green-600/60" },
                    ...
                    {isAdmin && (
                     ...
                     >
                       ...
                       >
                         ...
                         <div className="flex-1">
                           <span className={`text-lg font-medium ${isActive("/admin") ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-white"}`}>{t.adminPanel}</span>
                           <div className={`h-0.5 w-0 bg-gradient-to-r ${isActive("/admin") ? "from-red-400 to-red-600" : "from-red-400/50 to-red-600/50"} group-hover:w-full transition-all duration-300`}></div>
                         </div>
                       </Link>
                     </motion.div>
                    )}
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <div className={`h-0.5 w-0 bg-gradient-to-r ${isActive("/profile") ? "from-indigo-400 to-indigo-600" : "from-indigo-400/50 to-indigo-600/50"} group-hover:w-full transition-all duration-300`}></div>
                          </div>
                        </Link>
                      </motion.div>
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <div className={`h-0.5 w-0 bg-gradient-to-r ${isActive("/favorites") ? "from-indigo-400 to-indigo-600" : "from-red-400/50 to-red-600/50"} group-hover:w-full transition-all duration-300`}></div>
                          </div>
                        </Link>
                      </motion.div>
