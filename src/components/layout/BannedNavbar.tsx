    ...
    <>
      ...
      <nav className="hidden md:flex fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-6xl bg-white/60 dark:bg-gray-900/60 backdrop-blur-lg shadow-lg rounded-2xl border border-white/20 dark:border-gray-700/30 py-1.5 px-4 transition-all duration-300">
        <div className="flex justify-between items-center w-full">
          ...
          {/* القسم الأيمن - الإعدادات والحساب */}
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  <div className="space-y-1">
                    {[
                      ...
                      { href: "/contact", icon: "mail", label: t.contact, color: "from-cyan-400/60 to-cyan-600/60" },
                      { href: "/privacy-policy", icon: "privacy", label: t.privacy, color: "from-green-400/60 to-green-600/60" },
                      { href: "/terms-conditions", icon: "terms", label: t.terms, color: "from-blue-400/60 to-blue-600/60" }
                    ].map((item, index) => (
                      <motion.div
                        key={item.href}
                        initial={{ opacity: 0, x: -20 }}
