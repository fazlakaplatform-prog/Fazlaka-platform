  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setMessage({ type: 'error', text: result.error || 'Failed to create FAQ' });
      }
    } catch (error) {
      console.error('Error creating FAQ:', error);
      setMessage({ type: 'error', text: 'An error occurred while creating FAQ' });
    } finally {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Add New FAQ</h1>
        <p className="text-gray-600 dark:text-gray-400">Create a new FAQ with Arabic and English content</p>
      </div>
