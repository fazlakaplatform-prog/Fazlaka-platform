// src/app/admin/users/[id]/edit/components/EmailManagementSection.tsx
...
export default function EmailManagementSection({
  ...
  const handleSendVerificationLink = async (email: string, type: "primary" | "secondary", emailId?: string) => {
    ...
    try {
      const response = await fetch('/api/admin/users/send-verification-link', {
        method: 'POST',
        headers: {
                ...
                <div className="space-y-3 mb-4">
                  {secondaryEmails.map((email: SecondaryEmail) => (
                    <div key={email.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                      ...
                      <div className="mb-3">
                        {renderVerificationLinkSender(email.email, "secondary", email.id)}
                      </div>
                      
                      <div className="flex space-x-2">
                        <button
                          type="button"
                          onClick={() => handleMakePrimaryEmail(email.id)}
                        ...
                        {!email.isVerified && (
                          <button
                            type="button"
                            onClick={() => handleVerifyEmailDirectly(email.id)}
                            className="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300"
                          >
                            {t.verifyEmail}
                          </button>
                        ...
                        <button
                          type="button"
                          onClick={() => handleRemoveSecondaryEmail(email.id)}
