...
export default function EditUserPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [secondaryEmails, setSecondaryEmails] = useState<Array<{id: string, email: string, isVerified: boolean, verificationToken?: string, verificationTokenExpiry?: Date}>>([])
  ...
  const handleMakePrimaryEmail = async (emailId: string) => {
    try {
      const email = secondaryEmails.find(e => e.id === emailId)
      if (!email) return
      
      // Swap primary and secondary emails
      const updatedSecondaryEmails = [
        ...secondaryEmails.filter(e => e.id !== emailId),
        { id: Date.now().toString(), email: formData.email, isVerified: formData.emailVerified }
      ]
      
      const response = await fetch(`/api/admin/users/${id}/emails/${emailId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json"
        }
  ...
  const handleRemoveSecondaryEmail = async (emailId: string) => {
    try {
      const updatedSecondaryEmails = secondaryEmails.filter(e => e.id !== emailId)
  ...
  const handleVerifyEmailDirectly = async (emailId: string) => {
    try {
      ...
      const updatedSecondaryEmails = secondaryEmails.map(e => 
        e.id === emailId ? { ...e, isVerified: true } : e
      )
      
      const response = await fetch(`/api/admin/users/${id}/emails/${emailId}/verify`, {
        method: "PUT"
      })
