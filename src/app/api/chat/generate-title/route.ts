...
export async function POST(req: NextRequest) {
  try {
    ...
    // تحسين: استخدام نفس اسم الموديل من متغيرات البيئة أو الافتراضي
    const desiredModelName = process.env.GOOGLE_AI_MODEL || 'gemini-1.5-flash';
    
    const model = genAI.getGenerativeModel({ model: desiredModelName });
