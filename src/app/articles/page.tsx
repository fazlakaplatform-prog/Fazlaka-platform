...
import { 
  ...
  FaLightbulb, FaMicroscope, FaLaptopCode, FaPalette, FaChartLine, FaGlobe, FaAtom, FaMusic,
  FaSync
} from 'react-icons/fa';
...
interface Article {
  id: string; 
  slug: string;
  title: string; // Raw field from Prisma
  titleEn?: string; // Raw field from Prisma
  excerpt?: string;
  excerptEn?: string;
  featuredImageUrl?: string;
  featuredImageUrlEn?: string;
  localizedTitle?: string; // Computed field
  localizedExcerpt?: string; // Computed field
  localizedFeaturedImageUrl?: string; // Computed field
  episode?: {
    id: string; 
    slug: string;
    title: string;
  ...
  } | null; // Prisma relations can be null
  season?: {
    id: string; 
    slug: string;
    title: string;
...
export default function ArticlesPageClient() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [articlesRes, seasonsRes] = await Promise.all([
        fetch(`/api/articles?language=${language}`),
        fetch(`/api/seasons?language=${language}`)
      ]);

      if (!articlesRes.ok || !seasonsRes.ok) throw new Error('Failed to fetch data');

      const articlesData: { articles: Article[] } = await articlesRes.json();
      const seasonsData: { seasons: Season[] } = await seasonsRes.json();
      
      // --- Mapping logic for Prisma ---
      // Prisma returns raw fields (title, titleEn). We map them to localizedTitle for the UI.
      // تم إصلاح الخطأ: إزالة any واستخدام النوع المستنتج
      const articles = (articlesData.articles || []).map((a) => ({
        ...a,
        id: a.id, // Ensure id is present
        localizedTitle: language === 'ar' ? a.title : a.titleEn || a.title,
        localizedExcerpt: language === 'ar' ? a.excerpt : a.excerptEn || a.excerpt,
        localizedFeaturedImageUrl: language === 'ar' ? a.featuredImageUrl : a.featuredImageUrlEn || a.featuredImageUrl,
        season: a.season ? {
            ...a.season, 
            id: a.season.id, 
            localizedTitle: language === 'ar' ? a.season.title : a.season.titleEn || a.season.title 
        } : null,
        episode: a.episode ? { 
            ...a.episode, 
            id: a.episode.id, 
            localizedTitle: language === 'ar' ? a.episode.title : a.episode.titleEn || a.episode.title 
        } : null,
      }));
      ...
      const seasons = (seasonsData.seasons || []).map((s) => ({
        ...s,
        id: s.id,
        localizedTitle: language === 'ar' ? s.title : s.titleEn || s.title
      }));
      // -------------------------------
      ...
      seasons.forEach((season: Season) => {
        const seasonTitle = season.localizedTitle || 'Unknown Season';
        if (!grouped[seasonTitle]) grouped[seasonTitle] = [];
        // Updated comparison: use .id
        const seasonArticles = articles.filter(article => 
          article.season?.id === season.id
        );
        grouped[seasonTitle] = seasonArticles;
      });
      
      const articlesWithoutSeason = articles.filter(article => !article.season);
      if (articlesWithoutSeason.length > 0) {
        grouped[t.articlesWithoutSeason] = articlesWithoutSeason;
      }
      
      grouped[t.allArticles] = articles;
      
      setArticlesBySeason(grouped);
      
      const first = Object.keys(grouped)[0];
      if (first && first !== t.allArticles) {
        setOpenSeasons({ [first]: true });
      }
    } catch (err: unknown) {
      console.error(err);
      setError(t.error);
  ...
  }, [language, t.error, t.allArticles, t.articlesWithoutSeason, isUpdating]);
  ...
  }, [language, autoRefresh, loadData]);

  const toggleSeason = useCallback((title: string) => {
    setOpenSeasons((prev) => {
      const newState = { ...prev };
  ...
  const filteredBySeason = useMemo(() => {
    ...
    Object.entries(result).forEach(([season, articles]) => {
      ...
      const matches = articles.filter((article: Article) => {
        const title = article.localizedTitle?.toLowerCase() || '';
        const excerpt = article.localizedExcerpt?.toLowerCase() || '';
        return title.includes(q) || excerpt.includes(q);
      });
      if (matches.length > 0) out[season] = matches;
    });
    return out;
  }, [articlesBySeason, searchTerm, t.allArticles, filterSeason]);
  ...
  if (error) return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 pt-16">
      <div className="text-center bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl max-w-md">
        ...
        <button onClick={() => loadData()} className="mt-4 px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:opacity-90 transition-opacity">
          {t.retry}
        </button>
      </div>
    </div>
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 bg-white dark:bg-gray-800 rounded-full shadow-sm px-3 py-2 border border-gray-100 dark:border-gray-700 focus-within:ring-2 focus-within:ring-blue-200">
              ...
              />
              {searchTerm ? (
                <button onClick={() => setSearchTerm("")} className="flex items-center justify-center rounded-full p-1 hover:bg-gray-100 dark:hover:bg-gray-700 transition" aria-label={t.clearSearch} title={t.clearSearch}>
            ...
            <div className="flex flex-wrap gap-2 justify-between items-center">
              <div className="flex gap-2">
                <div className="inline-flex items-center rounded-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                  <button onClick={() => setViewMode("grid")} className={`flex items-center justify-center p-2 transition ${viewMode === "grid" ? "bg-purple-600 text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}`} aria-pressed={viewMode === "grid"} title={t.grid}>
                    <FaTh className={`h-5 w-5 ${viewMode === "grid" ? "text-white" : "text-gray-500 dark:text-gray-400"}`} />
                  </button>
                  <button onClick={() => setViewMode("list")} className={`flex items-center justify-center p-2 transition ${viewMode === "list" ? "bg-purple-600 text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}`} aria-pressed={viewMode === "list"} title={t.list}>
                    <FaList className={`h-5 w-5 ${viewMode === "list" ? "text-white" : "text-gray-500 dark:text-gray-400"}`} />
                  </button>
                </div>
                
                <button onClick={() => setShowFilters(!showFilters)} className={`px-3 py-2 rounded-md text-sm transition flex items-center justify-center shadow-md ${showFilters ? "bg-gradient-to-r from-indigo-600 to-indigo-700 text-white" : "bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700"}`}>
            ...
            {showFilters && (
              <div className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-md border border-gray-200 dark:border-gray-700">
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setFilterSeason(null)} className={`px-3 py-1 rounded-full text-sm transition ${filterSeason === null ? "bg-gradient-to-r from-blue-500 to-purple-500 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"}`}>
                    {t.allArticles}
                  </button>
                  {Object.keys(articlesBySeason).filter(season => season !== t.allArticles).map(season => (
                    <button key={season} onClick={() => setFilterSeason(season)} className={`px-3 py-1 rounded-full text-sm transition ${filterSeason === season ? "bg-gradient-to-r from-blue-500 to-purple-500 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600"}`}>
                      {season}
                    </button>
                  ))}
                </div>
                ...
                {viewMode === "grid" ? (
                  <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {searchResults.map((article: Article) => {
                      ...
                      return (
                        <motion.article key={article.id} variants={cardVariants} whileHover={{ scale: 1.02 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} layout className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col bg-white dark:bg-gray-800 shadow-md">
                          <Link href={`/articles/${encodeURIComponent(String(slug))}`} className="block group">
                            <div className="relative aspect-video bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800">
                              ...
                              <motion.div initial={{ opacity: 0 }} whileHover={{ opacity: 1, scale: 1.02 }} transition={{ duration: 0.18 }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                              ...
                              {articleSeason && (
                                <div className="mb-2">
                                  <span className="text-xs px-2 py-1 bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-900/50 dark:to-blue-800/50 text-blue-800 dark:text-blue-200 rounded-full">
                                    {t.season}: {articleSeason.localizedTitle}
                                  </span>
                                </div>
                              )}
                              ...
                              {episode && (
                                <div className="mb-2">
                                  <span className="text-xs px-2 py-1 bg-gradient-to-r from-green-100 to-green-200 dark:from-green-900/50 dark:to-green-800/50 text-green-800 dark:text-green-200 rounded-full">
                                    {t.episode}: {episode.localizedTitle}
                                  </span>
                                </div>
                              )}
                  ...
                  <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
                    {searchResults.map((article: Article) => {
                      ...
                      return (
                        <motion.div key={article.id} variants={cardVariants} whileHover={{ scale: 1.01 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} layout className="flex gap-4 items-center border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden p-4 hover:shadow-lg transition bg-white dark:bg-gray-800 shadow-md">
        ...
        <div className="space-y-6">
          {seasonEntries.length === 0 ? (
            ...
            seasonEntries.map(([seasonTitle, articles]) => {
              const isOpen = openSeasons[seasonTitle] === true;
              return (
                <div key={seasonTitle} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700/50">
                    ...
                    <div className="flex items-center gap-3">
                      <motion.button aria-expanded={isOpen} onClick={() => toggleSeason(seasonTitle)} whileTap={{ scale: 0.97 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-gradient-to-r from-blue-500 to-purple-500 text-white transition-all duration-300 hover:opacity-90 text-sm shadow-md">
                        <motion.span layout className="flex items-center gap-2">
                          <motion.span aria-hidden>
                            <motion.div initial={{ rotate: isOpen ? 180 : 0 }} animate={{ rotate: isOpen ? 180 : 0 }} transition={{ type: "spring", stiffness: 420, damping: 32 }} className="flex items-center">
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      <motion.div key={seasonTitle} initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.28, ease: "easeInOut" }} style={{ overflow: "hidden" }} className="px-4">
                        <motion.div layout className={`py-4 ${viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" : "space-y-4"}`}>
                          {articles.map((article: Article) => {
                            ...
                            return viewMode === "grid" ? (
                              <motion.article key={article.id} variants={cardVariants} initial="hidden" animate="visible" whileHover={{ scale: 1.02 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} layout className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col bg-white dark:bg-gray-800 shadow-md">
                                <Link href={`/articles/${encodeURIComponent(String(slug))}`} className="block group">
                                  <div className="relative aspect-video bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800">
                                    <ImageWithFallback src={thumbnailUrl} alt={title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                                    <motion.div initial={{ opacity: 0 }} whileHover={{ opacity: 1, scale: 1.02 }} transition={{ duration: 0.18 }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                              ...
                              <motion.div key={article.id} variants={cardVariants} initial="hidden" animate="visible" whileHover={{ scale: 1.01 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} layout className="flex gap-4 items-center border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden p-4 hover:shadow-lg transition bg-white dark:bg-gray-800 shadow-md">
                                <Link href={`/articles/${encodeURIComponent(String(slug))}`} className="flex items-center gap-4 flex-1 group">
                                  <div className="relative w-24 h-16 sm:w-32 sm:h-20 flex-shrink-0 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-lg overflow-hidden">
                                    <ImageWithFallback src={thumbnailUrl} alt={title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" fill sizes="240px" />
                                  </div>
