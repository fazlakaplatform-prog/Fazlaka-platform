...
export default function NotificationsPage() {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type.toUpperCase()) { // تحويل لأحرف كبيرة للتأكد
      case 'SUCCESS':
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {notifications.map((notification) => (
              <div key={notification.id} className={`bg-white dark:bg-gray-800 border rounded-md p-4 shadow-sm transition-all duration-300 ${notification.isRead ? 'border-gray-200 dark:border-gray-700' : 'border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/10'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center space-x-2 mr-2">
                        {!notification.isRead && (
                          <button onClick={() => handleMarkAsRead(notification.id)} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700" title={language === 'ar' ? 'تعيين كمقروء' : 'Mark as read'}>
                            <Check className="h-4 w-4 text-gray-500 dark:text-gray-400" />
                          </button>
                        )}
                        <button onClick={() => handleDelete(notification.id)} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700" title={language === 'ar' ? 'حذف' : 'Delete'}>
