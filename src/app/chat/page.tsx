...
interface ChatHistory {
  id: string; // تم التعديل من _id إلى id
  title: string;
  createdAt: string;
  updatedAt: string;
  shareId?: string;
...
export default function ChatPage() {
  ...
  const texts = {
    ar: {
      ...
      searchPromptTemplate: "أنا أبحث عن: \"{query}\". هل هذا المحتوى متاح على منصة فذلكة؟",
    ...
    try {
      ...
      if (response.ok) {
        ...
        const uniqueHistory = history.filter((chat: ChatHistory, index: number, self: ChatHistory[]) => 
          index === self.findIndex((c: ChatHistory) => c.id === chat.id)
        );
    ...
    try {
      ...
      if (response.ok) {
        ...
        setChatHistory(prev => prev.map(chat => 
          chat.id === currentChatId ? { ...chat, title: newChatTitle } : chat
        ));
  ...
  const generateShareLink = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setChatHistory(prev => prev.map(chat => 
          chat.id === currentChatId ? { ...chat, shareId: data.shareId, isPublic: data.isPublic } : chat
        ));
      }
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (messages.length === 0 && chatIdToUse) {
        ...
        setChatHistory(prev => prev.map(chat => 
          chat.id === chatIdToUse ? { ...chat, title: smartTitle } : chat
        ));
      }
  ...
  const handleSendMessageWithQuery = async (query: string, messagesToSend: Message[], chatId: string) => {
    ...
    try {
      ...
      if (messagesToSend.length === 1 && chatId) {
        ...
        setChatHistory(prev => prev.map(chat => 
          chat.id === chatId ? { ...chat, title: smartTitle } : chat
        ));
      }
