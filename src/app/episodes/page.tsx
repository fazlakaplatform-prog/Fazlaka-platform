...
// تعريف واجهات البيانات مع دعم اللغة لـ Prisma
interface Episode {
  id: string; // تم التغيير من _id
  slug: string;
  title: string; // حقل خام
  titleEn?: string; // حقل خام
  description?: string; // حقل خام
  descriptionEn?: string; // حقل خام
  thumbnailUrl?: string; // حقل خام
  thumbnailUrlEn?: string; // حقل خام
  localizedTitle?: string; // حقل محسوب
  localizedDescription?: string; // حقل محسوب
  localizedThumbnailUrl?: string; // حقل محسوب
  season?: {
    id: string; 
    slug: string;
    title: string;
...
export default function EpisodesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!episodesRes.ok || !seasonsRes.ok) throw new Error('Failed to fetch data');
      ...
      const rawSeasons = seasonsData.seasons || [];

      // تحويل البيانات الخام إلى بيانات مترجمة للواجهة
      // تم إصلاح الخطأ: استبدال any بـ Episode
      const episodes = rawEpisodes.map((ep: Episode) => ({
        ...ep,
        id: ep.id,
        localizedTitle: language === 'ar' ? ep.title : (ep.titleEn || ep.title),
        localizedDescription: language === 'ar' ? ep.description : (ep.descriptionEn || ep.description),
        localizedThumbnailUrl: language === 'ar' ? ep.thumbnailUrl : (ep.thumbnailUrlEn || ep.thumbnailUrl),
        season: ep.season ? {
          ...ep.season,
          id: ep.season.id,
          localizedTitle: language === 'ar' ? ep.season.title : (ep.season.titleEn || ep.season.title)
        } : null
      }));

      // تم إصلاح الخطأ: استبدال any بـ Season
      const seasons = rawSeasons.map((s: Season) => ({
        ...s,
        id: s.id,
      ...
      seasons.forEach((season: Season) => {
        ...
        const seasonEpisodes = episodes.filter((episode: Episode) => 
          episode.season?.id === season.id
        );
        grouped[seasonTitle] = seasonEpisodes;
      });
      
      const episodesWithoutSeason = episodes.filter((episode: Episode) => !episode.season);
      if (episodesWithoutSeason.length > 0) {
        grouped[t.episodesWithoutSeason] = episodesWithoutSeason;
      }
      
      grouped[t.allEpisodes] = episodes;
      
      setEpisodesBySeason(grouped);
      const first = Object.keys(grouped)[0];
      if (first && first !== t.allEpisodes && Object.keys(openSeasons).length === 0) {
    ...
    const setupEventSource = () => {
      ...
      eventSourceRef.current.onmessage = (event) => {
        try {
          ...
          if (data.type === 'change') {
            if (data.collection === 'episodes' || data.collection === 'seasons') {
              autoRefresh();
            }
          }
  ...
  const filteredBySeason = useMemo(() => {
    ...
    Object.entries(result).forEach(([season, episodes]) => {
      ...
      const matches = episodes.filter((episode: Episode) => {
        const title = episode.localizedTitle?.toLowerCase() || '';
        const description = episode.localizedDescription?.toLowerCase() || '';
        return title.includes(q) || description.includes(q);
      });
      if (matches.length > 0) out[season] = matches;
    });
    return out;
  }, [episodesBySeason, searchTerm, t.allEpisodes, filterSeason]);
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 bg-white dark:bg-gray-800 rounded-full shadow-sm px-3 py-2 border border-gray-100 dark:border-gray-700 focus-within:ring-2 focus-within:ring-blue-200">
              ...
              />
              {searchTerm ? (
                <button onClick={() => setSearchTerm("")} className="flex items-center justify-center rounded-full p-1 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
            ...
            <div className="flex flex-wrap gap-2 justify-between items-center">
              <div className="flex gap-2">
                <div className="inline-flex items-center rounded-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                  <button onClick={() => setViewMode("grid")} className={`flex items-center justify-center p-2 transition ${viewMode === "grid" ? "bg-blue-600 text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}`}>
                    <FaTh className={`h-5 w-5 ${viewMode === "grid" ? "text-white" : "text-gray-500 dark:text-gray-400"}`} />
                  </button>
                  <button onClick={() => setViewMode("list")} className={`flex items-center justify-center p-2 transition ${viewMode === "list" ? "bg-blue-600 text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}`}>
                ...
                {viewMode === "grid" ? (
                  <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {searchResults.map((episode: Episode) => {
                      ...
                      return (
                        <motion.article key={episode.id} variants={cardVariants} whileHover={{ scale: 1.02 }} layout className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col bg-white dark:bg-gray-800 shadow-md">
                              ...
                              {episodeSeason && (
                                <div className="mb-2">
                                  <span className="text-xs px-2 py-1 bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-900/50 dark:to-blue-800/50 text-blue-800 dark:text-blue-200 rounded-full">
                                    {t.season}: {episodeSeason.localizedTitle}
                                  </span>
                                </div>
                              )}
                          ...
                          <div className="mt-auto p-3 pt-1 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                            ...
                            <FavoriteButton contentId={episode.id} contentType="episode" />
                          </div>
                        </motion.article>
                      );
                    })}
                  ...
                  <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
                    {searchResults.map((episode: Episode) => {
                      ...
                      return (
                        <motion.div key={episode.id} variants={cardVariants} whileHover={{ scale: 1.01 }} layout className="flex gap-4 items-center border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden p-4 hover:shadow-lg transition bg-white dark:bg-gray-800 shadow-md">
                ...
                <div key={seasonTitle} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700/50">
                    ...
                    <div className="flex items-center gap-3">
                      <motion.button onClick={() => toggleSeason(seasonTitle)} whileTap={{ scale: 0.97 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-gradient-to-r from-blue-500 to-purple-500 text-white transition-all duration-300 hover:opacity-90 text-sm shadow-md">
                        ...
                        <motion.div layout className={`py-4 ${viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" : "space-y-4"}`}>
                          {episodes.map((episode: Episode) => {
                            ...
                            return viewMode === "grid" ? (
                              <motion.article key={episode.id} variants={cardVariants} initial="hidden" animate="visible" whileHover={{ scale: 1.02 }} layout className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col bg-white dark:bg-gray-800 shadow-md">
                                <Link href={`/episodes/${encodeURIComponent(String(slug))}`} className="block group">
                                  <div className="relative aspect-video bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800">
                                    <ImageWithFallback src={thumbnailUrl} alt={title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                                    <motion.div initial={{ opacity: 0 }} whileHover={{ opacity: 1, scale: 1.02 }} transition={{ duration: 0.18 }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                              ...
                              <motion.div key={episode.id} variants={cardVariants} initial="hidden" animate="visible" whileHover={{ scale: 1.01 }} layout className="flex gap-4 items-center border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden p-4 hover:shadow-lg transition bg-white dark:bg-gray-800 shadow-md">
                                <Link href={`/episodes/${encodeURIComponent(String(slug))}`} className="flex items-center gap-4 flex-1 group">
                                  <div className="relative w-24 h-16 sm:w-32 sm:h-20 flex-shrink-0 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-lg overflow-hidden">
                                    <ImageWithFallback src={thumbnailUrl} alt={title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" fill sizes="240px" />
                                  </div>
