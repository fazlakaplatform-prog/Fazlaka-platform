...
// مكون داخلي لاستخدام useSearchParams بشكل آمن
function SearchPageContent() {
  ...
  // تحديث دالة البحث لتقبل الفلاتر كمعامل لحل مشكلة Stale Closure
  ...
  const handleSuggestionClick = useCallback((suggestion: string) => {
    setQuery(suggestion);
    setShowSuggestions(false);
    // استخدام الفلاتر الحالية عند النقر على اقتراح
  ...
  const handleFilterChange = useCallback((newFilters: typeof filters) => {
    ...
    // تمرير الفلاتر الجديدة مباشرة للدالة
    if (query.trim()) {
  ...
  const loadTrendingSearches = useCallback(async () => {
    try {
      ...
      setResults(prev => ({
        // التأكد من وجود كائن نتائج صالح
  ...
  }, [initialQuery]); // إزالة handleSearch من الاعتماديات لتشغيلها مرة واحدة فقط عند التحميل
            ...
            {isLoading ? (
              ...
              <>
                ...
                {results.semanticResults.length > 0 ? (
                  ...
                  <div className="text-center py-12">
                    <div className="w-24 h-24 mx-auto mb-4 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center overflow-hidden">
                      <Image
