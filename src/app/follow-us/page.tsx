...
type PlatformType = 'youtube' | 'instagram' | 'facebook' | 'tiktok' | 'x' | 'twitter' | 
                   ...
                   'whatsapp' | 'telegram' | 'github' | 'behance' | 'dribbble' | 'discord';
...
function getSocialIcon(platform: string) {
  switch (platform) {
    ...
    case 'x': case 'twitter': return XIcon;
...
function getPlatformColor(platform: string) {
  switch (platform) {
    ...
    case 'x': case 'twitter': return 'from-gray-700 to-gray-900';
    case 'linkedin': return 'from-blue-600 to-blue-700';
    case 'threads': return 'from-gray-800 to-black';
    case 'snapchat': return 'from-yellow-400 to-yellow-500';
    case 'pinterest': return 'from-red-600 to-red-700';
    case 'reddit': return 'from-orange-500 to-orange-600';
    case 'whatsapp': return 'from-green-500 to-green-600';
    case 'telegram': return 'from-blue-400 to-blue-500';
    case 'github': return 'from-gray-700 to-gray-800';
    case 'behance': return 'from-blue-500 to-blue-600';
    case 'dribbble': return 'from-pink-400 to-pink-500';
...
const HeroSection = ({ language }: { language: string }) => {
  ...
  const scrollToPlatforms = () => {
    ...
    if (platformsSection) platformsSection.scrollIntoView({ behavior: 'smooth' });
  };
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-xl">
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-4 flex justify-center">
                      <button onClick={scrollToPlatforms} className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-4 py-2 rounded-full text-sm font-medium hover:from-indigo-600 hover:to-purple-600 transition-all duration-300">
                        {t.allPlatforms}
                      </button>
                    </div>
                  </div>
  ...
  useEffect(() => {
    const loadYouTubeData = async () => {
      try {
        ...
        if (data) setYoutubeData(data); else setError(true);
      } catch (err) {
    ...
    <section className="mb-12 sm:mb-16 relative">
      ...
      <div className="relative z-10 p-4 sm:p-6 md:p-8">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="group relative bg-white dark:bg-gray-800 rounded-3xl p-6 sm:p-8 shadow-xl border border-gray-200 dark:border-gray-700 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="flex flex-col justify-center">
                <div className="flex items-center mb-6">
                  {loading ? <div className="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded-full mr-4 animate-pulse"></div> : 
                   error || !youtubeData?.channelThumbnail ? <div className="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded-full mr-4 flex items-center justify-center"><FaYoutube className="text-gray-400 text-2xl" /></div> :
                   <Image src={youtubeData.channelThumbnail} alt={youtubeData.channelName} width={64} height={64} className="rounded-full mr-4" />}
                  <div>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{loading ? <div className="h-6 w-32 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></div> : error ? getPlatformName('youtube', language) : youtubeData?.channelName || getPlatformName('youtube', language)}</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{language === 'ar' ? 'قناة يوتيوب' : 'YouTube Channel'}</p>
                  </div>
                </div>
                {loading ? <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="text-center"><div className="text-xl font-bold text-gray-300 dark:text-gray-600 animate-pulse">---</div><div className="text-xs text-gray-400 dark:text-gray-500">{t.subscribers}</div></div>
                    <div className="text-center"><div className="text-xl font-bold text-gray-300 dark:text-gray-600 animate-pulse">---</div><div className="text-xs text-gray-400 dark:text-gray-500">{t.totalViews}</div></div>
                    <div className="text-center"><div className="text-xl font-bold text-gray-300 dark:text-gray-600 animate-pulse">---</div><div className="text-xs text-gray-400 dark:text-gray-500">{t.videos}</div></div>
                  </div> :
                  error ? <div className="text-center mb-6 text-red-500 dark:text-red-400">{t.error}</div> :
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="text-center"><div className="text-xl font-bold text-gray-900 dark:text-white">{youtubeData?.subscribers || '0'}</div><div className="text-xs text-gray-600 dark:text-gray-400">{t.subscribers}</div></div>
                    <div className="text-center"><div className="text-xl font-bold text-gray-900 dark:text-white">{youtubeData?.views || '0'}</div><div className="text-xs text-gray-600 dark:text-gray-400">{t.totalViews}</div></div>
                    <div className="text-center"><div className="text-xl font-bold text-gray-900 dark:text-white">{youtubeData?.videos || '0'}</div><div className="text-xs text-gray-600 dark:text-gray-400">{t.videos}</div></div>
                  </div>}
                <a href={youtubeLink.url} target="_blank" rel="noopener noreferrer" className="group relative inline-flex items-center justify-center overflow-hidden bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-bold py-3 px-6 rounded-full transition-all duration-300 transform hover:scale-105 shadow-lg">
                  <span className="relative z-10 flex items-center"><FaBell className={`mr-3 text-lg transition-transform duration-300 group-hover:${isRTL ? '-translate-x-1' : 'translate-x-1'}`} />{t.subscribe}</span>
                </a>
              </div>
              <div className="relative">
                <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center"><FaVideo className="mr-2 text-red-500" />{t.latestVideo}</h4>
                {loading ? <div className="relative rounded-xl overflow-hidden shadow-lg"><div className="w-full h-48 sm:h-56 bg-gray-200 dark:bg-gray-700 animate-pulse"></div></div> :
                 error || !youtubeData?.latestVideo ? <div className="relative rounded-xl overflow-hidden shadow-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center h-48 sm:h-56"><p className="text-gray-500 dark:text-gray-400">{t.error}</p></div> :
                 <div className="relative rounded-xl overflow-hidden shadow-lg group cursor-pointer" onClick={() => youtubeData.latestVideo && window.open(youtubeData.latestVideo.url, '_blank')}>
                   <Image src={youtubeData.latestVideo.thumbnail} alt={youtubeData.latestVideo.title} width={640} height={360} className="w-full h-48 sm:h-56 object-cover transition-transform duration-300 group-hover:scale-105" />
                   <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"><div className="bg-red-600 rounded-full p-3 transform scale-0 group-hover:scale-100 transition-transform duration-300"><FaPlay className="text-white text-xl" /></div></div>
                   <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4"><p className="text-white font-medium text-sm line-clamp-2">{youtubeData.latestVideo.title}</p></div>
                 </div>}
                {youtubeData?.latestVideo && <a href={youtubeData.latestVideo.url} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center text-red-600 dark:text-red-400 font-medium hover:text-red-700 dark:hover:text-red-300 transition-colors">{t.watchNow}<FaArrowRight className={`mr-2 ${isRTL ? 'mr-2 rotate-180' : 'ml-2'}`} /></a>}
              </div>
            </div>
          </div>
        </div>
...
const PlatformsSection = ({ socialLinks, isRTL, language }: { socialLinks: SocialLink[], isRTL: boolean, language: string }) => {
  const translations = {
    ar: { title: "منصّاتنا الرقمية", subtitle: "اختر المنصة التي تناسب احتياجاتك من مجموعتنا المتنوعة.", visit: "زيارة المنصة" },
    en: { title: "Our Digital Platforms", subtitle: "Choose the platform that suits your needs from our diverse collection.", visit: "Visit Platform" }
  };
  const t = translations[language === 'ar' ? 'ar' : 'en'];
  const filteredLinks = socialLinks.filter(link => !['mobile_app', 'desktop_app', 'app_store', 'google_play', 'download_link', 'website'].includes(link.platform));
    ...
    <section id="platforms-section" className="mb-12 sm:mb-16 relative">
      ...
      <div className="relative z-10 p-4 sm:p-6 md:p-8">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto text-sm sm:text-base animate-fade-in-up" style={{ animationDelay: '0.2s' }}>{t.subtitle}</p>
        </div>
        <div className={`grid ${getGridCols()} gap-6 sm:gap-8 max-w-6xl mx-auto`}>
          {filteredLinks.map((link, index) => {
            ...
            return (
              <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="group relative animate-fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
                ...
                <div className="relative bg-white dark:bg-gray-800 rounded-2xl p-6 sm:p-8 shadow-lg border border-gray-100 dark:border-gray-700 transition-all duration-300 hover:shadow-xl hover:-translate-y-2 h-full flex flex-col items-center justify-between min-h-[280px]">
                  <div className="flex flex-col items-center justify-center w-full">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2 text-center">{platformName}</h3>
                    <p className="text-gray-600 dark:text-gray-400 text-sm text-center mb-4">{language === 'ar' ? `انضم إلينا على ${platformName} لتكون جزءاً من مجتمعنا` : `Join us on ${platformName} to be part of our community`}</p>
                  </div>
...
const AppsSection = ({ socialLinks, isRTL, language }: { socialLinks: SocialLink[], isRTL: boolean, language: string }) => {
  const translations = {
    ar: { title: "تطبيقاتنا", subtitle: "حمل تطبيقاتنا الآن واستمتع بتجربة أفضل على جميع الأجهزة", mobileApps: "تطبيق الموبايل", desktopApps: "تطبيق الكمبيوتر", download: "تحميل", features: "المميزات", available: "متاح الآن", secure: "آمن 100%", fast: "سريع", free: "مجاني" },
    en: { title: "Our Applications", subtitle: "Download our app now and enjoy a better experience on all devices", mobileApps: "Mobile App", desktopApps: "Desktop App", download: "Download", features: "Features", available: "Available Now", secure: "100% Secure", fast: "Fast", free: "Free" }
  };
  const t = translations[language === 'ar' ? 'ar' : 'en'];
  const mobileApps = socialLinks.filter(link => ['mobile_app', 'app_store', 'google_play'].includes(link.platform));
  const desktopApps = socialLinks.filter(link => ['desktop_app', 'download_link', 'website'].includes(link.platform));
  const appData = {
    mobile: { icon: FaMobileAlt, color: 'from-blue-500 to-indigo-600', bgColor: 'from-blue-100 to-indigo-100 dark:from-blue-900/20 dark:to-indigo-900/20', features: [t.secure, t.fast, t.free] },
    desktop: { icon: FaDesktop, color: 'from-purple-500 to-pink-600', bgColor: 'from-purple-100 to-pink-100 dark:from-purple-900/20 dark:to-pink-900/20', features: [t.secure, t.fast, t.free] }
  };
  if (mobileApps.length === 0 && desktopApps.length === 0) return null;
  const getGridCols = () => { if (mobileApps.length > 0 && desktopApps.length > 0) return 'grid-cols-1 lg:grid-cols-2'; return 'grid-cols-1'; };
    ...
    <section className="mb-12 sm:mb-16 relative">
      ...
      <div className="relative z-10 p-4 sm:p-6 md:p-8">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent animate-fade-in-up">{t.title}</h2>
          <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto text-sm sm:text-base animate-fade-in-up" style={{ animationDelay: '0.2s' }}>{t.subtitle}</p>
        </div>
                ...
                <div className={`relative bg-gradient-to-br ${appData.mobile.bgColor} rounded-3xl p-6 sm:p-8 shadow-xl border border-blue-200 dark:border-blue-800 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2`}>
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{t.mobileApps}</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-400">iOS & Android</p>
                      </div>
                    </div>
                    <div className="flex gap-2"><FaApple className="text-gray-600 dark:text-gray-400 text-xl" /><FaGooglePlay className="text-gray-600 dark:text-gray-400 text-xl" /></div>
                  </div>
                ...
                <div className={`relative bg-gradient-to-br ${appData.desktop.bgColor} rounded-3xl p-6 sm:p-8 shadow-xl border border-purple-200 dark:border-purple-800 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2`}>
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{t.desktopApps}</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Windows, Mac, Linux</p>
                      </div>
                    </div>
                    <div className="flex gap-2"><FaWindows className="text-gray-600 dark:text-gray-400 text-xl" /><FaApple className="text-gray-600 dark:text-gray-400 text-xl" /><FaLinux className="text-gray-600 dark:text-gray-400 text-xl" /></div>
                  </div>
                  <div className="flex flex-wrap gap-2 mb-6">
...
function FollowUsContent() {
  ...
  useEffect(() => {
    ...
    if (savedLanguage !== null) detectedLanguage = savedLanguage;
    else { const browserLang = navigator.language || (navigator as unknown as { userLanguage: string }).userLanguage || ''; detectedLanguage = browserLang.includes('ar') ? 'ar' : 'en'; }
